import { Homeopath } from '@/types/services/service';
import { Phone, Award, CheckCircle2, Info, Star, MapPin, Pill, Building, Syringe } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

const HomeopathicDoctorCard = ({ data }: { data: Homeopath }) => {
  return (
    <div className="bg-white border border-teal-100 rounded-[2.5rem] p-6 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden group h-full flex flex-col">
      
      {/* Decorative Background Pattern */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-teal-50 rounded-bl-[5rem] group-hover:bg-teal-600 transition-colors duration-500 opacity-50 group-hover:opacity-10" />

      <div className="relative z-10 flex flex-col flex-1">
        {/* Top Header: Category & Status */}
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-2 bg-teal-600 text-white px-5 py-2.5 rounded-2xl shadow-lg shadow-teal-100">
        ...
        <div className="flex gap-5 items-center mb-6">
          <div className="relative shrink-0">
            <div className="w-20 h-20 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl ring-2 ring-teal-100 group-hover:ring-teal-500 transition-all relative bg-slate-100">
              <Image 
                src={data.profileImage === "#" ? "/default-doctor.png" : data.profileImage} 
                alt={data.name} 
                fill
          ...
          <div className="flex-1">
            ...
            <p className="text-teal-600 font-black text-[11px] uppercase mt-1">
        ...
        {/* Visit Fee Badge */}
        <div className="mb-6 flex items-center gap-2 bg-teal-50 border border-teal-100 px-4 py-2 rounded-2xl">
            <Syringe size={16} className="text-teal-600" />
            <span className="text-xs font-bold text-teal-800 tracking-tight">ভিজিট: {data.visitFee}</span>
        </div>

        {/* Info Grid (Experience & Location) */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex items-center gap-3">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <span className="text-xs font-black text-slate-800 truncate">{data.location}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-8">
          {data.speciality?.slice(0, 3).map((s, index) => (
            <span key={index} className="bg-teal-50/50 text-teal-700 px-3 py-1.5 rounded-xl text-[10px] font-bold border border-teal-100/50">
              🌿 {s}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-auto">
          ...
          <a 
            href={`tel:${data.phone}`} 
            className="flex-[1.8] py-4 bg-teal-600 text-white rounded-2xl text-[12px] font-black flex items-center justify-center gap-2 hover:bg-teal-700 transition-all active:scale-95 shadow-lg shadow-teal-100"
          >
...
export default HomeopathicDoctorCard;