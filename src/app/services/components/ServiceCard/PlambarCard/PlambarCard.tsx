import { Plumber } from '@/types/services/service';
import { Phone, Award, CheckCircle2, Info, Star, Droplets, Wrench, Clock, Home, Drill } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

const PlumberCard = ({ data }: { data: Plumber }) => {
  return (
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          {data.hasModernTools && (
            <div className="flex items-center gap-1.5 bg-sky-50 border border-sky-100 px-3 py-1.5 rounded-xl">
              <Drill size={12} className="text-sky-600" />
              <span className="text-[9px] font-black text-sky-700 uppercase">আধুনিক মেশিন</span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-5 items-center mb-6">
          <div className="relative shrink-0">
            <div className="w-20 h-20 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl ring-2 ring-blue-100 group-hover:ring-blue-500 transition-all relative bg-slate-100">
              <Image 
                src={data.profileImage === "#" ? "/default-plumber.png" : data.profileImage} 
                alt={data.name} 
                fill
          ...
          <div className="flex-1">
             <div className="flex items-center gap-1 mb-1 text-blue-600">
               <Wrench size={12} />
               <span className="text-[10px] font-black uppercase tracking-tighter">স্যানিটারি মিস্ত্রি</span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex items-center gap-3">
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">কল করার সময়</span>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {data.speciality?.slice(0, 3).map((s, index) => (
            <span key={index} className="bg-blue-50/50 text-blue-700 px-3 py-1.5 rounded-xl text-[10px] font-bold border border-blue-100/50">
              🚿 {s}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-auto">
          ...
          <a 
            href={`tel:${data.phone}`} 
            className="flex-[1.8] py-4 bg-blue-600 text-white rounded-2xl text-[12px] font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95 shadow-lg shadow-blue-100"
          >
