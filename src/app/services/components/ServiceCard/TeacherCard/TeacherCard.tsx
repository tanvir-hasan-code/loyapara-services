import { Teacher } from '@/types/services/service';
import { Phone, Award, CheckCircle2, Info, Star, GraduationCap, BookOpen, Clock, MapPin, Users, BookCheck } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

const TeacherCard = ({ data }: { data: Teacher }) => {
  return (
    <div className="bg-white border border-indigo-100 rounded-[2.5rem] p-6 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden group h-full flex flex-col">
      ...
      {/* Decorative Academic Background Pattern */}
      ...
      <div className="relative z-10 flex flex-col flex-1">
        {/* Top Header: Category & Batch Status */}
        <div className="flex justify-between items-center mb-8">
          ...
          {data.hasSmallBatch && (
            <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-xl">
              ...
              <span className="text-[9px] font-black text-blue-700 uppercase">ব্যাচ সুবিধা</span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-5 items-center mb-6">
          <div className="relative shrink-0">
            <div className="w-20 h-20 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl ring-2 ring-indigo-100 group-hover:ring-indigo-500 transition-all relative bg-slate-100">
              <Image 
                src={data.profileImage === "#" ? "/default-teacher.png" : data.profileImage} 
                alt={data.name} 
                fill
          ...
          <div className="flex-1">
             <div className="flex items-center gap-1 mb-1 text-indigo-600">
               <BookCheck size={12} />
               <span className="text-[10px] font-black uppercase tracking-tighter">{data.education}</span>
            </div>
        ...
        {/* Target Class Badge */}
        <div className="mb-6 flex items-center gap-2 bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-2xl">
            <BookOpen size={16} className="text-indigo-600 shrink-0" />
            <span className="text-xs font-bold text-indigo-800 tracking-tight">শ্রেণী: {data.targetClass}</span>
        </div>

        {/* Info Grid (Experience & Teaching Time) */}
        ...
        {/* Speciality Tags (Subjects) */}
        <div className="flex flex-wrap gap-2 mb-8">
          {data.speciality?.slice(0, 3).map((s, index) => (
            <span key={index} className="bg-indigo-50/50 text-indigo-700 px-3 py-1.5 rounded-xl text-[10px] font-bold border border-indigo-100/50">
              📚 {s}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-auto">
          ...
          <a 
            href={`tel:${data.phone}`} 
            className="flex-[1.8] py-4 bg-indigo-600 text-white rounded-2xl text-[12px] font-black flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-100"
          >
